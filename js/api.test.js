...
describe('api.js', () => {
  ...
  describe('handleApiError', () => {
    test('should throw rate limit error for 403', async () => {
      const response = {
        status: 403,
        json: () => Promise.resolve({ message: 'API rate limit exceeded' })
      };

      await expect(handleApiError(response)).rejects.toThrow('Rate limit exceeded');
    });
    ...
    test('should throw not found error for 404', async () => {
      ...
      await expect(handleApiError(response)).rejects.toThrow('Not found');
    });

    test('should throw invalid query error for 422', async () => {
      const response = {
        status: 422,
        json: () => Promise.resolve({ message: 'Validation Failed' })
      };

      await expect(handleApiError(response)).rejects.toThrow('Invalid query');
    });

    test('should throw generic error for 500', async () => {
      const response = {
        status: 500,
