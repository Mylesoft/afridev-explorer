...
describe('utils.js', () => {
  ...
  describe('getLanguageColor', () => {
    ...
    test('should return default gray for unknown languages', () => {
      expect(getLanguageColor('UnknownLang')).toBe('#858585');
    });

    test('should handle null input', () => {
      expect(getLanguageColor(null)).toBe('#858585');
    });
  });
  ...
  describe('detectBioKeywords', () => {
    test('should detect open to work', () => {
      expect(detectBioKeywords('open to work, fullstack dev').isOpenToWork).toBe(true);
    });

    test('should detect co-founder', () => {
      expect(detectBioKeywords('I am a co-founder looking to build').isCoFounder).toBe(true);
    });

    test('should detect hiring', () => {
      expect(detectBioKeywords("we're hiring engineers").isHiring).toBe(true);
    });

    test('should handle empty bio', () => {
      const result = detectBioKeywords('');
      expect(result.isOpenToWork).toBe(false);
      expect(result.isCoFounder).toBe(false);
  ...
  describe('Bookmark functions', () => {
    ...
    test('toggleBookmark adds username to bookmarks', () => {
      toggleBookmark('torvalds');
      expect(isBookmarked('torvalds')).toBe(true);
    });

    test('toggleBookmark removes username on second call', () => {
      toggleBookmark('torvalds');
      toggleBookmark('torvalds');
      expect(isBookmarked('torvalds')).toBe(false);
    });

    test('getBookmarks returns array of bookmarked usernames', () => {
      toggleBookmark('user1');
      toggleBookmark('user2');
      expect(getBookmarks()).toContain('user1');
      expect(getBookmarks()).toContain('user2');
    });

    test('is Bookmarked returns false for non-bookmarked user', () => {
  ...
  describe('Search history functions', () => {
    ...
    test('addToSearchHistory saves search term', () => {
      addToSearchHistory('Python');
      expect(getSearchHistory()).toContain('Python');
    });

    test('getSearchHistory returns array of searches', () => {
      addToSearchHistory('Python');
      addToSearchHistory('JavaScript');
      expect(getSearchHistory()).toHaveLength(2);
    });

    test('getSearchHistory limits to 10 items', () => {
      for (let i = 0; i < 15; i++) {
    ...
    test('addToSearchHistory ignores empty strings', () => {
      addToSearchHistory('');
